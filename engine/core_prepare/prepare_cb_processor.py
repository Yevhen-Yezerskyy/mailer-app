# FILE: engine/core_prepare/prepare_cb.py  (обновлено — 2025-12-29)
# Смысл (правки 2025-12-29):
# - ПОЛНОСТЬЮ убраны entity-lock'и (per city/branch) и reserve-фаза.
# - Эксклюзивность батча обеспечивается атомарным POP из entity-очереди под kind-lock в _pop_batch().
# - kind-lock держим только на время работы с кеш-очередями; во время GPT/DB он НЕ держится.
# - Печать/логика очередей/rotate/jump сохранены.
        ...
        while tries > 0 and q:
            ...
            if eq:
                if random.random() <= DO_PROB:
                    # ВАЖНО: эксклюзивность обеспечивается атомарным "вырезанием" из очереди под kind-lock.
                    take = eq[:BATCH_SIZE]
                    rest = eq[BATCH_SIZE:]
...
def task_prepare_geo() -> Dict[str, Any]:
    ...
    try:
        ...
        # work (батч уже эксклюзивен, потому что вырезан из очереди под kind-lock)
        rating_id = int(st["rating_id"])
        task_id = int(st["task_id"])
        target_hash = int(st["target_hash"])
        ...
        try:
            _p(kind, f"BATCH rating_id={rating_id} task_id={task_id} take={len(ids)}")
            ...
            if not candidates:
                _p(kind, f"NOOP rating_id={rating_id} task_id={task_id} reason=no_candidates_in_db")
                return {"mode": "noop", "step": "no_candidates_in_db"}
