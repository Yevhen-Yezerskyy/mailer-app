# FILE: web/mailer_web/admin_views/branches_11880.py  (новое — 2026-02-10)
# PURPOSE: "Бранчи 11880": поиск/фильтр/сортировка/пэйджинг (200), toggle is_active, bulk GPT-fill label_rus.
...
TABLE = "public.branches_raw_11880"
...
def _bulk_fill_label_rus(ids: list[int]) -> int:
    if not ids:
        return 0
    ...
    for branch_id in ids:
        with connection.cursor() as cur:
            cur.execute(f"SELECT label, label_rus FROM {TABLE} WHERE id=%s", [branch_id])
            row = cur.fetchone()
            if not row:
                continue
            label = str(row[0] or "").strip()
            label_rus = str(row[1] or "").strip()

        if not label or label_rus:
            continue

        resp = gpt.ask(
            tier="nano",
            instructions="Переведи на русский кратко и нейтрально (1–5 слов). Без кавычек, без точки. Только перевод.",
            input=label,
            use_cache=True,
            user_id="admin:branches_11880:label_rus",
            service_tier="flex",
        )
        ...
        with transaction.atomic():
            with connection.cursor() as cur:
                cur.execute(
                    ...
                    UPDATE {TABLE}
                    SET label_rus=%s
                    WHERE id=%s AND (label_rus IS NULL OR label_rus='')
                    """,
                    [out, branch_id],
                )
                if cur.rowcount:
...
def view(request: HttpRequest) -> HttpResponse:
    # PRG redirect target (чтобы не было повторных POST)
    ...
    if request.method == "POST":
        ...
        if (request.POST.get("bulk_action") or "").strip() == "gpt_fill":
            raw_ids = request.POST.getlist("selected")
            ids = sorted({i for i in (_to_int(x, 0) for x in raw_ids) if i > 0})
            if not ids:
                messages.warning(request, "Ничего не выбрано.")
            else:
                n = _bulk_fill_label_rus(ids)
                messages.success(request, f"GPT заполнил label_rus: {n} шт.")
    ...
    return render_admin(
        request,
        template="admin/serenity/branches_11880.html",
        context={
            "title": "Бранчи 11880",
            "q": q,
            "active": active,
...
page = AdminPage(
    route="serenity/branches-11880/",
    name="serenity_branches_11880",
    title="Бранчи 11880",
    nav_section="Serenity",
    nav_label="Бранчи 11880",
    view=view,
)
