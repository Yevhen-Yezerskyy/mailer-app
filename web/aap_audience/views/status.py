# FILE: web/aap_audience/views/status.py  (обновлено) 2025-12-15
# Fix: counts(city/branch) из crawl_tasks + companies_count из raw_contacts_gb через queue_sys(status='collected').
...
def status_view(request):
    ...
    if task_ids:
        with connection.cursor() as cur:
            ...
            # найдено компаний по raw_contacts_gb через queue_sys(collected)
            cur.execute(
                """
